...
require('fs').readdirSync(normalizedPath).forEach(file => {
  const extensionlessFilename = file.split('.js')[0]
...
const SlowmodeFilter = (msg) => {
  if (guildCfg.slowmodedChannels.filter(smChannel => msg.channel.id === smChannel.id).length > 0) {
    const channel = guildCfg.slowmodedChannels.filter(smChannel => msg.channel.id === smChannel.id)[0]

    if (channel.immune && channel.immune.find(id => id === msg.author.id)) {
      return false
    }

    const slowmodeTimePeriod = new Date(new Date().getTime() - channel.rate * 1000)
    const slowmodeViolation = msg.channel.messages.cache.find(filteredMessages => {
    ...
    if (slowmodeViolation && slowmodeViolation !== msg) {
      const timeRemaining = () => {
        const diff = slowmodeViolation.createdAt.getTime() - slowmodeTimePeriod.getTime()
    ...
    buildStructs(msg, msg.guild.id, msg.member.id, (PermStruct, CommandStruct) => {
      ...
      if (CommandStruct.command === 'help' && CommandStruct.args.length === 0) {
        ...
        Object.keys(client.CommandRegistry).forEach(command => {
          ...
          if (!ResponseStruct.hasOwnProperty(client.CommandRegistry[command].Category)) { ResponseStruct[client.CommandRegistry[command].Category] = [] }
