...
module.exports.getRandomRedditPost = async (Subreddit) => {
  ...
  const attempsLimit = 16
  let attempts = 0
  const bannedWords = ['serious', 'nsfw', 'sex', 'cum', 'masturbate', 'porn', 'nigger', 'kike', 'spic', 'zipperhead', 'faggot', 'chink', 'retard', 'dago', 'tranny', 'paki', 'erection', 'fetish']
  do {
    const post = await RedditClient.getSubreddit(Subreddit).getRandomSubmission()
    console.log(post)
...
module.exports.getGuildMemberFromCommandStruct = (CommandStruct) => {
  ...
  if (!member) {
    ...
    const guildNickMatch = msg.guild.members.cache
      .filter(m => m.nickname)
    ...
    if (!member) {
      ...
      const discordUsernameMatch = msg.guild.members.cache.find(m => m.user.username.toLowerCase().includes(CommandStruct.args[0].toLowerCase()))
