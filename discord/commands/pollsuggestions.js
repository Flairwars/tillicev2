...
module.exports.run = async (CommandStruct, PermStruct) => {
  ...
  const suggestionMessages = await suggestionsChannel.messages.fetch({ limit: 100 })
  let acceptedSuggestions = 0

  const suggestions = suggestionMessages.filter(msg => msg.author === client.Client.user) // Filter messages not made by the bot
  ...
  if (CommandStruct.args.length > 0) {
    const ExcludedSuggestions = parseInt(CommandStruct.args[0])
  ...
  suggestions.forEach(suggestion => {
    ...
    const embed = new EmbedBuilder(suggestionEmbed)
