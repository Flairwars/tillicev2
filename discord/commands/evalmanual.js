...
module.exports.run = (CommandStruct, PermStruct) => {
  ...
  const suggestionsID = CommandStruct.args[2].toLowerCase()
  const channelID = CommandStruct.args[1].toLowerCase()
  const acceptOrReject = CommandStruct.args[0].toLowerCase()
  ...
  if (acceptOrReject === 'accept' || acceptOrReject === 'deny') {
    suggestionPollChannel.messages.fetch(suggestionsID).then(message => {
      const newPollEmbed = new EmbedBuilder(message.embeds[0])
