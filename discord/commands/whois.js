...
module.exports.run = async (CommandStruct, PermStruct) => {
  ...
  const member = helpers.getGuildMemberFromCommandStruct(CommandStruct)
  ...
  const redditName = member.nickname.split(prefixRegex).pop()
...
function buildDiscordEmbed (member) {
  ...
  embed.addFields([
    ...
    { name: 'Roles', value: roles || 'No roles assigned' }
  ])
...
async function buildRedditEmbed (member, redditUser) {
  ...
  const trophies = (await redditUser.getTrophies()).trophies

  const accountCreated = await redditUser.created_utc
  const redditAge = new Date((accountCreated) * 1000).toDateString()
  ...
  // Build the basis of the embed
  const embed = new EmbedBuilder()
    .setColor(fwInfo[flair].colourHex)
    .setTitle(member.user.tag)
    .setThumbnail(member.user.avatarURL())
    .setFooter({ text: `ID: ${member.user.id}` })
