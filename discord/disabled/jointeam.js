...
module.exports.run = async (CommandStruct, PermStruct) => {
  ...
  const guild = client.guilds.cache.get('463794005231271976') // MEGASERVER : 463794005231271976 // BOT-TESTING GUILD : 809491629207191572
  const member = guild.member(CommandStruct.message.author)
  const userNick = member ? member.displayName : null

  const teamChosen = CommandStruct.args.join(' ')
  ...
  const assignedRole = CommandStruct.message.guild.roles.cache.find(role => role.name.toLowerCase() === teamRoles[choice])
